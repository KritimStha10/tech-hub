        ...
        <article class="sign-up">
            <div class="row d-flex justify-content-center">
                <div class="col-md-3">
                    ...
                    <p class="sign-up__subtitle">Reset Password</p>
                    @include('errors.error')
                    @include('success.success')
                    {!! Form::open(['url' => 'reset-password','method' => 'POST', 'class' => 'sign-up-form form']) !!}
                    <label class="form-label-wrapper">
                        <p class="form-label">Email</p>
                        <input name="email" class="form-input" type="email" placeholder="Enter your email" required value="{{request('email')}}" readonly/>
                        <input  name="token" class="form-input" type="hidden" placeholder="Enter your email" required value="{{$token}}" readonly/>
                    </label>
                    <label class="form-label-wrapper">
